            ...
            {pageItems.map((task, i) => {
              ...
              return (
                <div key={task.id} style={{ display: 'grid', gridTemplateColumns: '1.8fr 1.1fr 1fr 0.8fr 1.1fr 0.8fr 0.8fr', padding: '13px 22px', alignItems: 'center', borderBottom: i < pageItems.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{task.title}</div>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 13, color: 'var(--text-secondary)' }}>{assignee?.name}</div>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 13, color: 'var(--text-secondary)' }}>{team?.name.replace("'s Team", '')}</div>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: 'var(--text-secondary)' }}>{task.priority}</div>
                  <div><StatusBadge status={task.status} /></div>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--heading)' }}>{task.progress}%</div>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: bucketOf(task) === 'overdue' ? 'var(--amber-text)' : 'var(--text-muted)' }}>{formatDate(task.dueDate)}</div>
                </div>
              );
            })}
            {filtered.length === 0 && (
...
function ReportStat({ value, label, color = 'var(--heading)' }) {
  return (
    <Card style={{ textAlign: 'center' }}>
      <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 22, color }}>{value}</div>
      <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 11.5, color: 'var(--text-muted)', marginTop: 4 }}>{label}</div>
    </Card>
  );
}
