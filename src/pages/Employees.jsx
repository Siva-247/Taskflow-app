...
// Admin/Super Admin manage the whole company's roster from Settings'
// Members section now, so this page is Manager-only — their own
// department's Employees/Interns, unchanged from before.
export default function Employees() {
  const { currentUser, users, teams, departments, tasks, statsFor, setUserActive, deleteUser } = useApp();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 14, color: 'var(--text-secondary)', marginTop: 4 }}>
            {`Employees across ${myDepartment?.name || 'your department'}`}
          </div>
        </div>
      ...
      <Pagination page={page} totalItems={managerRows.length} onChange={setPage} />
