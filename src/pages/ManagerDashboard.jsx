  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: 720 }}>
            ...
            {teamRows.map((row, i) => (
              ...
              >
                <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13.5, color: 'var(--text-primary)' }}>{row.team.name}</div>
                <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 13.5, color: 'var(--text-secondary)' }}>{row.lead?.name}</div>
                <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13.5, color: 'var(--heading)' }}>{row.total}</div>
                <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13.5, color: 'var(--heading)' }}>{row.completed}</div>
                <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13.5, color: 'var(--heading)' }}>{row.inProgress}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--heading)', width: 34, textAlign: 'right' }}>{row.completionRate}%</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {overallTopPerformers.length > 0 && (
        <Card>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)' }}>Top performers</div>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: 'var(--text-muted)', marginTop: 2 }}>
            Ranked by average marks across graded work
          </div>

          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 11.5, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--text-muted)', marginTop: 20 }}>
          ...
          {topPerformersByTeam.length > 0 && (
            <>
              <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 11.5, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--text-muted)', marginTop: 24 }}>
                By team
              </div>
              <div className="responsive-grid" style={{ display: 'grid', '--cols': `repeat(${Math.min(topPerformersByTeam.length, 2)}, 1fr)`, gap: 20, marginTop: 10 }}>
                {topPerformersByTeam.map(({ team, top3 }) => (
                  <div key={team.id} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
                    <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13.5, color: 'var(--text-primary)', marginBottom: 10 }}>{team.name}</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {top3.map((p, i) => (
                        <div key={p.user.id} onClick={() => navigate(`/tasks?assignee=${p.user.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                          ...
                          <span style={{ flex: 1, fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{p.user.name}</span>
                          <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13, color: 'var(--accent-dark)' }}>{p.avgMarks}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="responsive-grid" style={{ display: 'grid', '--cols': '1fr 1fr', gap: 20 }}>
        <Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: 14 }}>
            {overdueTasks.length === 0 && (
              <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontSize: 13.5, color: 'var(--text-muted)', padding: '11px 0' }}>Nothing overdue right now.</div>
            )}
              ...
              return (
                <div key={task.id} onClick={() => navigate(`/tasks/${task.id}`)} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '11px 0', borderTop: '1px solid var(--border)', cursor: 'pointer' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
        ...
        <Card>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)' }}>Recent department activity</div>
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: 14 }}>
            {deptActivity.length === 0 && <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontSize: 13.5, color: 'var(--text-muted)', padding: '11px 0' }}>Nothing yet.</div>}
      ...
      {trend.length > 0 && (
        <Card>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)', marginBottom: 18 }}>Completion trend</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {trend.map((week) => (
              <div key={week.label} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 110, flexShrink: 0, fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{week.label}</div>
                ...
                <div style={{ width: 38, textAlign: 'right', fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--heading)' }}>{week.pct}%</div>
              </div>
            ))}
          </div>
        </Card>
...
function RankBadge({ rank, size = 26 }) {
  return (
    <div style={{
      ...
      fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: size * 0.5,
    }}>
      {rank}
    </div>
  );
...
function PerformerCard({ rank, performer, onClick }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13.5, color: 'var(--text-primary)' }}>{performer.user.name}</div>
        <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{performer.teamName}</div>
      </div>
      <div>
        <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 20, color: 'var(--accent-dark)' }}>{performer.avgMarks}%</span>
