    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
        <div>
          ...
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 14, color: 'var(--text-secondary)', marginTop: 4 }}>Here's your work for today.</div>
        </div>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 16.5, color: 'var(--heading)' }}>My tasks</div>
          <div onClick={() => navigate('/tasks')} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13.5, color: 'var(--accent-dark)' }}>View all</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div key={task.id} style={{ padding: '20px 0', borderTop: '1px solid var(--border)', marginTop: i === 0 ? 16 : 0 }}>
              <div className="stack-mobile" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div style={{ cursor: 'pointer' }} onClick={() => navigate(`/tasks/${task.id}`)}>
                  <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 16, color: 'var(--text-primary)' }}>{task.title}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 8 }}>
                    <PriorityBadge priority={task.priority} />
                    <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: 'var(--text-muted)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 16 }}>
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 15, color: 'var(--heading)' }}>{task.progress}%</span>
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: 'var(--text-muted)', marginLeft: 8 }}>{task.status}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 12 }}>
                {(() => {
                  ...
                  return (
                    <span
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 12.5,
                ...
                <span onClick={() => toggleCommentBox(task.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 12.5, color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  <IconComment size={13} />
          ...
          {openTasks.length === 0 && (
            <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontSize: 13.5, color: 'var(--text-muted)', padding: '20px 0' }}>All caught up — no open tasks.</div>
          )}
        </div>
      </Card>
      ...
      <div className="responsive-grid" style={{ display: 'grid', '--cols': '1fr 0.85fr 0.85fr', gap: 20 }}>
        ...
        <Card>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)' }}>Upcoming deadlines</div>
          {nextDeadline ? (
            <div onClick={() => navigate(`/tasks/${nextDeadline.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16, cursor: 'pointer' }}>
              ...
              <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 13.5, color: 'var(--text-primary)' }}>
                {nextDeadline.title}{nextDeadline.dueDate ? ` — ${nextDeadline.dueDate.slice(5)}` : ''}
              </span>
            </div>
          ) : (
            <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontSize: 13.5, color: 'var(--text-muted)', marginTop: 16 }}>Nothing coming up.</div>
          )}
        </Card>

        <Card>
          <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)' }}>My marks</div>
          {markedTasks.length === 0 ? (
            <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontSize: 13.5, color: 'var(--text-muted)', marginTop: 16 }}>No marks yet.</div>
          ) : (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 12, color: 'var(--text-muted)' }}>Average</span>
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 15, color: 'var(--accent-dark)' }}>{averageMarks}%</span>
              </div>
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: markedTasks.length ? 14 : 0 }}>
            {markedTasks.slice(0, 3).map((t, i) => (
              ...
              >
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 12.5, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title}</span>
                <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13, color: 'var(--heading)', flexShrink: 0 }}>{t.marks}%</span>
              </div>
            ))}
          </div>
        </Card>
...
function StatChip({ value, label, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 17, color }}>{value}</span>
