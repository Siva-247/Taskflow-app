    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
        <div>
          ...
          <div style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 500, fontSize: 16, color: 'var(--text-secondary)', marginTop: 4 }}>Here's your work for today.</div>
        </div>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 600, fontSize: 18.5, color: 'var(--heading)' }}>My tasks</div>
          <div onClick={() => navigate('/tasks')} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--accent-dark)' }}>View all</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div key={task.id} style={{ padding: '20px 0', borderTop: '1px solid var(--border)', marginTop: i === 0 ? 16 : 0 }}>
              <div className="stack-mobile" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div style={{ cursor: 'pointer' }} onClick={() => navigate(`/tasks/${task.id}`)}>
                  <div style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 600, fontSize: 18, color: 'var(--text-primary)' }}>{task.title}</div>
                ...
                <Button variant={task.status === STATUS.TODO ? 'secondary' : 'primary'} onClick={() => navigate(`/tasks/${task.id}`)} style={{ padding: '8px 20px', fontSize: 15 }}>
                  {task.status === STATUS.PENDING_APPROVAL ? 'View' : task.status === STATUS.TODO ? 'Start' : 'Update Progress'}
                </Button>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 16 }}>
                <span style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 700, fontSize: 17, color: 'var(--heading)' }}>{task.progress}%</span>
                <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 500, fontSize: 14.5, color: 'var(--text-muted)', marginLeft: 8 }}>{task.status}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 12 }}>
                {(() => {
                  ...
                  return (
                    <span
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 600, fontSize: 14.5,
                ...
                <span onClick={() => toggleCommentBox(task.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 600, fontSize: 14.5, color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  <IconComment size={13} />
              ...
              {openCommentTaskId === task.id && (
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div>
                    <Button variant="secondary" style={{ padding: '7px 18px', fontSize: 14.5 }} disabled={!commentDraft.trim()} onClick={() => postComment(task.id)}>
                      Post comment
                    </Button>
                  </div>
                </div>
          ...
          {openTasks.length === 0 && (
            <div style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontSize: 15.5, color: 'var(--text-muted)', padding: '20px 0' }}>All caught up — no open tasks.</div>
          )}
        </div>
      </Card>
      ...
      <div className="responsive-grid" style={{ display: 'grid', '--cols': '1fr 0.85fr 0.85fr', gap: 20 }}>
        <Card>
          <div style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 600, fontSize: 17.5, color: 'var(--heading)' }}>Today's work update</div>
          <div style={{ marginTop: 13, padding: '13px 16px', border: '1px solid var(--border)', borderRadius: 10, background: 'var(--field-bg)', minHeight: 46, display: 'flex', alignItems: 'center' }}>
            <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 500, fontSize: 15.5, color: 'var(--text-muted)' }}>What did you work on today?</span>
          </div>
        ...
        <Card>
          <div style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 600, fontSize: 17.5, color: 'var(--heading)' }}>Upcoming deadlines</div>
          {nextDeadline ? (
            <div onClick={() => navigate(`/tasks/${nextDeadline.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16, cursor: 'pointer' }}>
              ...
              <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 500, fontSize: 15.5, color: 'var(--text-primary)' }}>
                {nextDeadline.title}{nextDeadline.dueDate ? ` — ${nextDeadline.dueDate.slice(5)}` : ''}
              </span>
            </div>
          ) : (
            <div style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontSize: 15.5, color: 'var(--text-muted)', marginTop: 16 }}>Nothing coming up.</div>
          )}
        </Card>

        <Card>
          <div style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 600, fontSize: 17.5, color: 'var(--heading)' }}>My marks</div>
          {markedTasks.length === 0 ? (
            <div style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontSize: 15.5, color: 'var(--text-muted)', marginTop: 16 }}>No marks yet.</div>
          ) : (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 600, fontSize: 14, color: 'var(--text-muted)' }}>Average</span>
                <span style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 700, fontSize: 17, color: 'var(--accent-dark)' }}>{averageMarks}%</span>
              </div>
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: markedTasks.length ? 14 : 0 }}>
            {markedTasks.slice(0, 3).map((t, i) => (
              ...
              >
                <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 500, fontSize: 14.5, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title}</span>
                <span style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 700, fontSize: 15, color: 'var(--heading)', flexShrink: 0 }}>{t.marks}%</span>
              </div>
            ))}
          </div>
        </Card>
...
function StatChip({ value, label, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "'Poppins',system-ui,sans-serif", fontWeight: 700, fontSize: 19, color }}>{value}</span>
      <span style={{ fontFamily: "'Manrope',system-ui,sans-serif", fontWeight: 600, fontSize: 13.5, color: 'var(--text-muted)' }}>{label}</span>
    </div>
  );
}
