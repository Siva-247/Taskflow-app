  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div>
        ...
        <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 14, color: 'var(--text-secondary)', marginTop: 4 }}>Organization reference</div>
      </div>

      <Card>
        <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 15.5, color: 'var(--heading)' }}>Organization structure</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
          {departments.map((dept) => (
            <div key={dept.id}>
              <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 700, fontSize: 13.5, color: 'var(--text-primary)' }}>{dept.name}</div>
                ...
                {teams.filter((t) => t.departmentId === dept.id).map((team) => {
                  ...
                  return (
                    <div key={team.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 500, fontSize: 13, color: 'var(--text-secondary)' }}>
      ...
      <Card style={{ background: 'var(--field-bg)', border: '1px dashed var(--border)' }}>
        <div style={{ fontFamily: "'Outfit',system-ui,sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
