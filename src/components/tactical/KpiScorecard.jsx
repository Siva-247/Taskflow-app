...
export default function KpiScorecard({ scorecard, canEdit, periodFrom, periodTo, onManualEntrySaved }) {
  ...
  const scoreTone = scorecard.weightedScore === null ? null
    : scorecard.weightedScore >= 90 ? BAND_TONE.EPI
      : scorecard.weightedScore >= 75 ? BAND_TONE.MPI
        : BAND_TONE.LPI;
    ...
    <div style={{ border: '1px solid var(--line)', borderRadius: 14, padding: '16px 18px', background: 'var(--surface)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 20, color: scoreTone ? scoreTone.fg : 'var(--text-muted)' }}>
            {scorecard.weightedScore !== null ? `${scorecard.weightedScore}%` : '—'}
          </div>
          <div style={{ fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 10, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{scorecard.weightMeasuredPct}% of weight measured</div>
        </div>
      </div>
      <div>
